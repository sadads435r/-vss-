...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        self.diagnostic_request_count = 0
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            if self.diagnostic_request_count % 30 == 0:
                top_scores = sorted(scores.reshape(-1), reverse=True)[:10]
                print(
                    "[GDINO_SCORE_DIAG] "
                    f"request={self.diagnostic_request_count} "
                    f"max={top_scores[0]:.6f} "
                    f"top10={','.join(f'{score:.6f}' for score in top_scores)}",
                    flush=True,
                )
